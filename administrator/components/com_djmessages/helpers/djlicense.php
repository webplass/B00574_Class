...
class DJLicense {
	...
	public static function getSubscription($name){
		...
		if (!in_array('curl', get_loaded_extensions())) {
			return self::renderAlert(JText::_('COM_DJMESSAGES_DJLIC_CURL_NOT_INSTALLED'), 'error');
		}
		...
		if(strstr($res[0], 'E')){
			...
			if($t!='license' && $t!='Savelicense'){
				if($license==''){
					//$app->enqueueMessage(JText::_('COM_DJMESSAGES_DJLIC_ENTER_LICENSE'),'Error');
				}else{
			...
			$update .= '<div class="djlic_line djll1">'.JText::_('COM_DJMESSAGES_DJLIC_INSTALLED_VER').' <span>'.$version.'</span></div>';
			$update .= '<div class="djlic_line djll2">'.JText::_('COM_DJMESSAGES_DJLIC_AUTHOR').' <a target="_blank" href="https://www.dj-extensions.com"><span>DJ-EXTENSIONS</span></a></div>';
			$update .= '<div class="djlic_line djll3">'.JText::_('COM_DJMESSAGES_DJLIC_LAST_VERSION_AVAILABLE').' <span>'.(empty($res[1]) ? '-' : $res[1]).'</span></div>';
			...
			if($res[0]=='E4'){
				...
				$update .= '<span class="djlic_iline4">'.JText::_('COM_DJMESSAGES_DJLIC_RENEW_INFO').'</span>';
				$update .='</a>';
				$update .= '<p><code>'.$license.'</code></p>';
				$update .= '<p><span class="djlic_iline5">'.JText::_('COM_DJMESSAGES_DJLIC_INVALID_INFO').'</span></p>';
				$update .= '<p><a class="btn btn-warning" href="https://dj-extensions.com/my-licenses" target="_blank">'.JText::_('COM_DJMESSAGES_DJLIC_RENEW_YOUR_LICENSE_KEY_HERE').'</a></p>';
				$update .= '<span class="djlic_iline6">'.JText::_('COM_DJMESSAGES_DJLIC_OR').'</span>';
				$update .= '<span class="djlic_iline1">'.JText::_('COM_DJMESSAGES_DJLIC_ENTER_ANOTHER_LICENSE_CODE_FOR').'</span>';
				...
				$update .= '<span class="djlic_iline1">'.JText::_('COM_DJMESSAGES_DJLIC_ENTER_LICENSE_CODE_FOR').'</span>';
				$update .= '<span class="djlic_iline2"><code>'.$u->getHost().'</code></span>';
				...
				$update .= '<span class="djlic_iline3">'.JText::_('COM_DJMESSAGES_DJLIC_INVALID_INFO').'</span>';
			...
			$update .= '<div class="djlic_line djll1">'.JText::_('COM_DJMESSAGES_DJLIC_INSTALLED_VER').' <span>'.$version.'</span></div>';
			$update .= '<div class="djlic_line djll2">'.JText::_('COM_DJMESSAGES_DJLIC_AUTHOR').' <a target="_blank" href="https://www.dj-extensions.com"><span>DJ-EXTENSIONS</span></a></div>';
			$update .= '<div class="djlic_line djll3">'.JText::_('COM_DJMESSAGES_DJLIC_LAST_AVAILABLE_VER').' <span>'.(empty($res[3]) ? '-' : $res[3]).'</span></div>';
			$update .= '<div class="djlic_separator"></div>';
