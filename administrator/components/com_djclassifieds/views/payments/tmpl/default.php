...
?>
<form action="index.php?option=com_djclassifieds&view=payments" method="post" name="adminForm" id="adminForm">
	...
	?>
        <tr>
			...
			<td >
                <?php  
                	if($payment->type==4){                   
						echo '<a href="index.php?option=com_djclassifieds&task=item.edit&id='.(int) $payment->i_order_id.'">'.$payment->i_order_name.'</a>';           
						echo '<br />Seller: '.$payment->u_order_name.' ('.$payment->u_order_id.')<br />';           
						echo $payment->u_order_email;                 
					}else if($payment->type==3){
			...
			<td>
				...
				if($payment->coupon){
					echo '<br />'.JText::_('COM_DJCLASSIFIEDS_COUPON').': <a target="_blank" href="index.php?option=com_djclassifieds&view=coupons&filter_search='.$payment->coupon.'">'.$payment->coupon.'</a>';
					echo '<br />'.JText::_('COM_DJCLASSIFIEDS_DISCOUNT').': -'.$payment->coupon_discount;
				}
