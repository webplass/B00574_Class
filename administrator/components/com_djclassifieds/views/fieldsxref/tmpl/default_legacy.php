...
<form action="index.php?option=com_djclassifieds&view=fieldsxref&tmpl=component" method="post" name="adminForm" id="adminForm" enctype='multipart/form-data'>
	...
	<table class="adminlist" id="fieldsxref">
		<thead>
			<tr>
	           <th width="10%">
...
<?php 
	...
	foreach($this->fields as $f){
		...
		echo '</td><td>';
			echo '<input style="'.$st.'" type="text" class="fieldsordering" id="fieldsordering_'.$f->id.'" name="fieldsordering_'.$f->id.'" size="3" value="'.$f->ordering.'" />';
		echo '</td><tr>';
		if($ord<$f->ordering){
			$ord=$f->ordering;
		}
