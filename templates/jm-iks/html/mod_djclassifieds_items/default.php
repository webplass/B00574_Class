		...
		<div class="items items-cols<?php echo $cols; ?>">
			<div class="items-col icol<?php echo $col_nr; ?>"><div class="icol-in">
					...
					foreach($items as $i){
						...
						$img_width = ($params->get('img_width','') ? $params->get('img_width','') : '');
						$img_height = ($params->get('img_height','') ? $params->get('img_height','') : '');

						echo '<div class="item'.$item_class.'" style="max-width:'.$img_width.'px;">';
						...
						if($params->get('show_img')==1){
							if(count($i->images)){
								...
								echo '<img src="'.JURI::base().$thumb.'" alt="'.str_ireplace('"', "'", $i->name).'" title="'.$i->images[0]->caption.'" width="'.$img_width.'" height="'.$img_height.'" />';
								echo '</a>';
