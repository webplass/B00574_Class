...
class JFormFieldDJUpdater extends JFormField
{
	...
	protected function getInput()
	{
		...
		$path 	= '/plugins/content/djsuggester'; 
		...
		self::setUpdateServer('Suggester', $pro);
	...
	public static function getSubscription($ext, $pro){
		...
		$ch = curl_init();
		$db = JFactory::getDBO();
		$query = "SELECT manifest_cache FROM #__extensions WHERE name='".$ext."'";
		$db->setQuery($query);
		...
		if($pro) {
			$html  = '<h4>'.JText::_('PLG_DJSUGGESTER_PRO_DESC').'  '.$version.'</h4>';
		} else {
			$html  = '<h4>'.JText::_('PLG_DJSUGGESTER_LIGHT_DESC').'  '.$version.'</h4>';
		}
		...
		if(strstr($res[0], 'E')){
			...
			if(!$pro) $html .= JText::sprintf('PLG_DJSUGGESTER_GET_PRO_LINK', JText::_('DJUPDATER_BUY_LICENSE'));
	...
	private function save($ext){
		...
		if(strstr($res[0], 'E')){
			$query = "UPDATE #__update_sites SET extra_query='' WHERE name='DJ-Suggester' AND type='extension' ";
			$db->setQuery($query);
			$db->query();
			...
			$query = "SELECT manifest_cache FROM #__extensions WHERE name='plg_content_djsuggester' ";
			$db->setQuery($query);
			$mc = json_decode($db->loadResult());
			$version = $mc->version;
			
			$extra_query = 'license='.$license.'&v='.$version.'&site='.JURI::root();
			$query = "UPDATE #__update_sites SET extra_query='".addslashes($extra_query)."' WHERE name='DJ-Suggester' AND type='extension' ";
			$db->setQuery($query);
			$db->query();
	...
	public static function setUpdateServer($name = 'Suggester', $pro = false) {
		...
		$query = "SELECT extension_id, manifest_cache FROM #__extensions WHERE name='plg_content_dj".strtolower($name)."' ";
		$db->setQuery($query);
		$pkg = $db->loadObject();
