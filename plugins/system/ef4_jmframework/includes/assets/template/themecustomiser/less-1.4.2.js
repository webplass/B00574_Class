...
if (typeof environment === "object" && ({}).toString.call(environment) === "[object Environment]") {
	...
	// https://github.com/ringo/ringojs/issues/88
...
less.Parser = function Parser(env) {
    ...
    var imports = this.imports = {
        ...
        push: function (path, currentFileInfo, callback) {
            ...
            less.Parser.importer(path, currentFileInfo, function (e, root, fullPath) {
                parserImporter.queue.splice(parserImporter.queue.indexOf(path), 1); // Remove
																					// the
																					// path
																					// from
																					// the
                ...
                parserImporter.files[fullPath] = root;                        // Store
																				// the
																				// root
    ...
    return parser = {
        ...
        // Rule ("border", Value ([Expression [Dimension 1px][Keyword
		// "solid"][Color #000]]))
        // Rule ("width", Value ([Expression [Operation "+" [Variable
		// "@w"][Dimension 4px]]]))
        // Ruleset (Selector [Element '>', '.child'], [...])
        // ])
        //
        // In general, most rules will try to parse a token with the `$()`
		// function, and if the return
        // value is truly, will return a new node, of the relevant type.
        ...
        parsers: {
            ...
            //
            // The recursive nature of the grammar is an interplay between the
			// `block`
            // rule, which represents `{ ... }`, the `ruleset` rule, and this
			// `primary` rule,
            ...
            entities: {
                ...
                quoted: function () {
                    ...
                    if (input.charAt(j) === '~') { j++, e = true } // Escaped
																	// strings
                ...
                // rgb(255, 0, 255)
                //
                // We also try to catch IE's `alpha()`, but let the `alpha`
				// parser
                // deal with the details.
                //
                // The arguments are parsed with the `entities.arguments`
				// parser.
                //
                call: function () {
                ...
                // Parse url() tokens
                //
                // We use a specific rule for urls, because they don't really
				// behave like
                // standard function calls. The difference is that the argument
				// doesn't have
                // to be enclosed within a string, so it can't be parsed as an
				// Expression.
                //
                url: function () {
                    var value;
                ...
                // #4F3C2F
                //
                // `rgb` and `hsl` colors are parsed through the `entities.call`
				// parser.
                //
                color: function () {
                    var rgb;
            ...
            //
            // The variable part of a variable definition. Used in the `rule`
			// parser
            //
            // @fink:
            //
            ...
            mixin: {
                ...
                definition: function () {
                    ...
                    if (match = $(/^([#.](?:[\w-]|\\(?:[A-Fa-f0-9]{1,6} ?|[^A-Fa-f0-9]))+)\s*\(/)) {
                        ...
                        // .mixincall("@{a}");
                        // looks a bit like a mixin definition.. so we have to
						// be nice and restore
                        if (!$(')')) {
                            furthest = i;
                            restore();
            ...
            // because the `block` rule will be expecting it, but we still need
			// to make sure
            ...
            rule: function (tryAnonymous) {
                ...
                if (name = $(this.variable) || $(this.property)) {
                    // prefer to try to parse first if its a variable or we are
					// compressing
            ...
            expression: function () {
                ...
                while (e = $(this.addition) || $(this.entity)) {
                    entities.push(e);
                    // operations do not allow keyword "/" dimension (e.g.
					// small/20px) so we support that here
                    if (!peek(/^\/[\/*]/) && (delim = $('/'))) {
...
if (less.mode === 'browser' || less.mode === 'rhino') {
    ...
    less.Parser.importer = function (path, currentFileInfo, callback, env) {
        ...
        // This is so we can get the syntax tree as opposed to just the CSS
		// output,
    ...
    "data-uri": function(mimetypeNode, filePathNode) {
        ...
        if (fileSizeInKB >= DATA_URI_MAX_KB) {
            ...
            if (this.env.ieCompat !== false) {
                ...
                // if explicitly disabled (via --no-ie-compat on CLI, or
				// env.ieCompat === false), merely warn
...
// these static methods are used as a fallback when the optional 'mime'
...
tree.Ruleset.prototype = {
    ...
    toCSS: function (env) {
        ...
        for (var i = 0; i < this.rules.length; i++) {
            ...
            if (rule.rules || (rule instanceof tree.Media)) {
                ...
                // Output only the first @charset definition as such - convert
				// the others
                // to comments in case debug is enabled
                if (rule.name === "@charset") {
                    // Only output the debug info together with subsequent
					// @charset definitions
                    // a comment (or @media statement) before the actual
					// @charset directive would
                    // be considered illegal css as it has to be on the first
					// line
                    if (env.charset) {
                        if (rule.debugInfo) {
        ...
        for (i = 0; i < selector.elements.length; i++) {
            ...
            if (el.value !== "&") {
                ...
                for(j = 0; j < newSelectors.length; j++) {
                    sel = newSelectors[j];
                    // if we don't have any parent paths, the & might be in a
					// mixin so that it can be used
                    // whether there are parents or not
                    if (context.length == 0) {
                        // the combinator used on el should now be applied to
						// the next element instead so that
                        // it is not lost
                        if (sel.length > 0) {
                            sel[0].elements = sel[0].elements.slice(0);
                            sel[0].elements.push(new(tree.Element)(el.combinator, '', 0)); // new
																							// Element(el.Combinator,
																							// ""));
                        }
                        ...
                        for(k = 0; k < context.length; k++) {
                            ...
                            // then join the last selector's elements on to the
							// parents selectors
                            ...
                            // construct the joined selector - if & is the first
							// thing this will be empty,
                            // if not newJoinedSelector will be the last set of
                            ...
                            if (parentSel.length > 0) {
                                ...
                                // join the elements so far with the first part
								// of the parent
    ...
    mergeElementsOnToSelectors: function(elements, selectors) {
        ...
        for(i = 0; i < selectors.length; i++) {
            ...
            // if the previous thing in sel is a parent this needs to join on to
			// it
            if (sel.length > 0) {
...
(function (tree) {
    ...
    var parseCopyProperties = [
        'paths',            // option - unmodified - paths to search for imports
							// on
        'optimization',     // option - optimization level (for the chunker)
        'files',            // list of files that have been imported, used for
							// import-once
        ...
        'processImports',   // option - whether to process imports. if false
							// then imports will not be imported
        'syncImport',       // option - whether to import synchronously
        'mime',             // browser only - mime type for sheet import
        'currentFileInfo'   // information about the current file - for error
							// reporting and importing and making urls relative
							// etc.
    ];
    ...
    var evalCopyProperties = [
        ...
        'yuicompress', // whether to compress with the outside tool yui
						// compressor
    ...
    tree.extendFinderVisitor.prototype = {
        ...
        visitRuleset: function (rulesetNode, visitArgs) {
            ...
            // get &:extend(.a); rules which apply to all selectors in this
			// ruleset
            ...
            // now find every selector and apply the extends that apply to all
			// extends
    ...
    tree.processExtendsVisitor.prototype = {
        ...
        doExtendChaining: function (extendsList, extendsListTarget, iterationCount) {
            //
            // chaining is different from normal extension.. if we extend an
			// extend then we are not just copying, altering and pasting
            // the selector we would do normally, but we are also adding an
			// extend with the same target selector
            // this means this new extend can then go and alter other extends
            //
            // this method deals with all the chaining work - without it, extend
			// is flat and doesn't work on other extend selectors
            // this is also the most expensive.. and a match on one selector can
			// cause an extension of a selector we had already processed if
            ...
            // a target extend is the one on the ruleset we are looking at
			// copy/edit/pasting in place
            // e.g. .a:extend(.b) {} and .b:extend(.c) {} then the first extend
			// extends the second one
            // and the second is the target.
            // the seperation into two lists allows us to process a subset of
			// chains with a bigger set, as is the
            ...
            for(extendIndex = 0; extendIndex < extendsList.length; extendIndex++){
                for(targetExtendIndex = 0; targetExtendIndex < extendsListTarget.length; targetExtendIndex++){
                    ...
                    // find a match in the target extends self selector (the bit
					// before :extend)
                    ...
                    if (matches.length) {
                        ...
                        extend.selfSelectors.forEach(function(selfSelector) {
                            ...
                            // add the extend onto the list of extends for that
							// selector
							...
							// :extend(.a,.b) then multiple
                            // extends will look at the same selector path, so
							// when extending
                            // we know that any others will be duplicates in
							// terms of what is added to the css
            ...
            if (extendsToAdd.length) {
                ...
                // now process the new extends on the existing rules so that we
				// can handle a extending b extending c ectending d extending
				// e...
        ...
        visitRuleset: function (rulesetNode, visitArgs) {
            ...
            // look at each selector path in the ruleset, find any extend
			// matches and then copy, find and replace
        ...
        findMatch: function (extend, haystackSelectorPath) {
            //
            // look through the haystack selector path to try and find the
			// needle - extend.selector
            ...
            for(haystackSelectorIndex = 0; haystackSelectorIndex < haystackSelectorPath.length; haystackSelectorIndex++) {
                ...
                for(hackstackElementIndex = 0; hackstackElementIndex < hackstackSelector.elements.length; hackstackElementIndex++) {
                    ...
                    // if we allow elements before our match we can add a
					// potential match every time. otherwise only at the first
					// element.
                    ...
                    for(i = 0; i < potentialMatches.length; i++) {
                        ...
                        // selectors add " " onto the first element. When we use
						// & it joins the selectors together, but if we don't
                        // then each selector in haystackSelectorPath has a
						// space before it added in the toCSS phase. so we need
						// to work out
                        ...
                        // if null we remove, if not, we are still valid, so
						// either push as a valid match or continue
                        if (potentialMatch) {
                            if (potentialMatch.finished) {
                                potentialMatch.length = needleElements.length;
                                potentialMatch.endPathIndex = haystackSelectorIndex;
                                potentialMatch.endPathElementIndex = hackstackElementIndex + 1; // index
																								// after
																								// end
																								// of
																								// match
                                potentialMatches.length = 0; // we don't
																// allow matches
																// to overlap,
																// so start
																// matching
        ...
        extendSelector:function (matches, selectorPath, replacementSelector) {
            ...
            // for a set of matches, replace each match with the replacement
			// selector
...
function loadStyleSheet(sheet, callback, reload, remaining) {
    ...
    // sheet may be set to the stylesheet for the initial load or a collection
    ...
    if (env.relativeUrls) {
        // todo - this relies on option being set on less object rather than
		// being passed in as an option
    ...
    xhr(href, sheet.type, function (data, lastModified) {
    	// JM initial variables
    	data = JM_initial_vars + data;
            ...
            try {
                ...
                new(less.Parser)(env).parse(data, function (e, root) {
                    ...
                    try {
                        ...
                        // TODO - there must be a better way? A generic
						// less-to-css function that can both call error
...
function extractId(href) {
    return href.replace(/^[a-z-]+:\/+?[^\/]+/, '' )  // Remove protocol &
														// domain
               .replace(/^\//,                 '' )  // Remove root /
               .replace(/\.[a-zA-Z]+$/,        '' )  // Remove simple
														// extension
               .replace(/[^\.\w-]+/g,          '-')  // Replace illegal
														// characters
...
function xhr(url, type, callback, errback) {
    ...
    if (isFileProtocol && !less.fileAsync) {
    	...
    	response = JMhandleResponse(xhr);
        handleResponse(xhr, response, callback, errback);
    }

    function handleResponse(xhr, jm_response, callback, errback) {
        if (xhr.status >= 200 && xhr.status < 300) {
            callback(jm_response,
...
function JMhandleResponse(xhr) {
	var theme_vars = JMgetCookie('JM_theme_vars_' + JM_tpl_name);
	console.log(theme_vars);
	var response = xhr.responseText;
	if (theme_vars && response) {
		response += theme_vars;
	}
	return response;
}

function JMgetCookie(c_name)
{
