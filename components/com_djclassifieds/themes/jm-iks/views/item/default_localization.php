			...
			<div id="google_map_box" style="display:none;">
				...
				<?php if($par->get('show_gm_driving')==1){ ?>
					...
					<form action="<?php echo JURI::base();?>/index.php" method="post" class="gm_drive_dir" target="_blank">
						...
						<input class="button btn" type="submit" value="<?php echo JText::_('COM_DJCLASSIFIEDS_DIRECTIONS_SEARCH'); ?>" />
						<?php 								
						...
						<?php 
							if(isset($_COOKIE["djcf_latlon"])) {														
								...
								echo '<a class="gm_drive_dir_l btn readmore" target="_blank" href="https://maps.google.com/maps?saddr='.$saddr.'&daddr='.$item_address.'">'.JText::_('COM_DJCLASSIFIEDS_OR_USE_LOCALIZATION').'<span></span></a>';
							}else{
								echo '<span class="gm_drive_dir_l"><button class="btn readmore" onclick="getDJDriveLocation()" >'.JText::_('COM_DJCLASSIFIEDS_OR_USE_LOCALIZATION').'</button><span></span></span>';
							}
						?>
