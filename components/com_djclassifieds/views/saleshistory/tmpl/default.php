...
<div class="orders_history">
	<div class="title_top"><h1><?php echo JText::_("COM_DJCLASSIFIEDS_SALES_HISTORY"); ?></h1></div>
	<div class="dj-items-table2">
		...
		foreach($this->orders as $order){ 
			...
			$event_results = $dispatcher->trigger('onAfterDJClassifiedsDisplaySalesItem', array (&$order, & $par, 'saleshistory'));			
			...
			echo '<div class="item_row row'.$row.'">';
				if($order->i_name){
					echo '<div class="item_col icon first"  style="width:'.$icon_col_w.'px"  >';
						echo '<a href="'.DJClassifiedsSEO::getItemRoute($order->item_id.':'.$order->i_alias,$order->cat_id.':'.$order->c_alias,$order->region_id.':'.$order->r_name).'">';
					...
					echo '<div class="item_col name">';					
						echo '<h3><a class="title" href="'.DJClassifiedsSEO::getItemRoute($order->item_id.':'.$order->i_alias,$order->cat_id.':'.$order->c_alias,$order->region_id.':'.$order->r_name).'" >'.$order->i_name.'</a></h3>';
					echo '</div>';
				}else{
					echo '<div class="item_col icon first"  style="width:'.$icon_col_w.'px"  >';
						echo '<img src="'.JURI::base().'/components/com_djclassifieds/assets/images/no-image.png" ';
				...
				<div class="item_col"> 
					...
					<?php if(isset($order->extra_info)){
						echo $order->extra_info;
					}?>
				</div>	
				<div class="item_col"> 
					<?php 
						...
						echo '<div class="profile_email_outer">';
							echo '<div class="user_pd_outer" style="display:none" ><div id="user_pd_'.$order->id.'">';
								...
								echo $user_pd;
							echo '</div></div>';
							echo '<a class="profile_email button modal" title="'.JText::_('COM_DJCLASSIFIEDS_BUYER_DETAILS').'" href="#user_pd_'.$order->id.'" rel="{size: {x: 480, y: 320}, ajaxOptions: {method: &quot;get&quot;}}" >'.JText::_('COM_DJCLASSIFIEDS_CONTACT_BUYER').'</a>';
						echo '</div>';	
						...
						//echo '<div class="profile_email_outer"><a class="profile_email button" href="mailto:'.$order->email.'">'.JText::_('COM_DJCLASSIFIEDS_CONTACT_BUYER').'</a></div>';
					?>						
				</div>
				<?php 
			echo '</div>';
