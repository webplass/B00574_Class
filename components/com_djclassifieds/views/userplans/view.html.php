...
class DJClassifiedsViewUserplans extends JViewLegacy{

	public function __construct($config = array())
	{
		parent::__construct($config);
		$this->_addPath('template', JPATH_COMPONENT.  '/themes/default/views/userplans');
		$par = JComponentHelper::getParams( 'com_djclassifieds' );
		$theme = $par->get('theme','default');
		if ($theme && $theme != 'default') {
			$this->_addPath('template', JPATH_COMPONENT.  '/themes/'.$theme.'/views/userplans');
		}
	}	
	...
	public function display($tpl = null)
	{
		...
		if($user->id=='0'){
			...
			$plans_expired= $model->getExpiredPlans();
			//$count_plans = $model->getCountPlans();
			...
			$promotions = $model->getPromotions();
				...
				$pagination = new JPagination( $count_plans, $limitstart, $limit );*/
				
			$this->assignRef('plans', $plans);
			$this->assignRef('plans_expired', $plans_expired);
